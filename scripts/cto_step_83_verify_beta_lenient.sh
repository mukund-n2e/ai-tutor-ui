...
curl -s "$BASE/onboarding" | grep -E -o 'data-ssr-beta(="[^"]*")?' || echo "miss"

echo -n "SESSION_BETA: "
curl -s "$SESSION_URL" | grep -E -o 'data-ssr-beta(="[^"]*")?' || echo "miss"

if curl -s "$BASE/onboarding" | grep -q 'data-ssr-beta' \
   && curl -s "$SESSION_URL" | grep -q 'data-ssr-beta'; then
  echo "BETA_OK"
else
