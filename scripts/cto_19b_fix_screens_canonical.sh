...
SC_DIR="$APP_APP_DIR/screens"
USC_DIR="$APP_APP_DIR/__screens"
LOG_DIR="./.cto_logs"; mkdir -p "$LOG_DIR"
TS="$(date -u +%Y%m%d_%H%M%S)"
BUILD_LOG="$LOG_DIR/step19b_canonical_build_${TS}.log"
BR="wp019b-screens-canonical-${TS}"
...
mkdir -p "$SC_DIR/[slug]" "$USC_DIR/[slug]"

# --- Write canonical /screens pages (server components) ---
cat > "$SC_DIR/page.tsx" <<'TSX'
...
# --- One-way alias: __screens re-exports from /screens (no reverse alias) ---
cat > "$USC_DIR/page.tsx" <<'TSX'
export { metadata } from '../screens/page';
export { default } from '../screens/page';
TSX

cat > "$USC_DIR/[slug]/page.tsx" <<'TSX'
...
git add "$SC_DIR" "$USC_DIR"
git commit -m "fix(screens): canonicalize to /screens and make __screens one-way alias (break cycles)" >/dev/null 2>&1 || true
...
if command -v gh >/dev/null 2>&1; then
  ...
  [ -z "$PR_URL" ] && PR_URL="$(gh pr create --head "$BR" --title "fix(screens): canonical /screens + __screens alias (no cycles)" --body "Implements screens at /screens and re-exports at /__screens to avoid circular re-exports.")"
fi

echo "=== CTO 19b CANONICAL SCREENS SUMMARY START ==="
echo "App dir: $APP_DIR"
echo "Build: PASS (log: $BUILD_LOG)"
[ -n "$PR_URL" ] && echo "PR: $PR_URL" || [ -n "$GH_URL" ] && echo "Compare: $GH_URL/compare/$BR?expand=1"
echo "Routes: /screens , /screens/[slug]  (+ alias /__screens, /__screens/[slug])"
echo "=== CTO 19b CANONICAL SCREENS SUMMARY END ==="
