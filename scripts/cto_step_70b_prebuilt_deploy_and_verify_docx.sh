...
# Link + PULL PROD settings WITHOUT prompts
vercel link --project "$PROJECT" --yes --scope "$TEAM" >/dev/null
vercel pull --yes --environment=production --scope "$TEAM" >/dev/null

# Build locally using project settings (Root Directory = web)
vercel build --scope "$TEAM"

# Deploy the prebuilt artifacts to Production (avoids git-author guard)
vercel deploy --prebuilt --prod --scope "$TEAM"

# Verify export endpoint: headers + magic bytes (PK)
echo "— headers —"
