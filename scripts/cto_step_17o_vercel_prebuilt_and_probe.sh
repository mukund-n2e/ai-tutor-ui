...
# Confirm login and link again in THIS shell/context
if ! vercel whoami >/dev/null 2>&1; then
  die "Not logged in in this shell. Run: vercel login"
fi

if [ ! -f "$APP_DIR/.vercel/project.json" ]; then
  die "Not linked in $APP_DIR. Run inside that dir: vercel link (choose existing project that serves tutorweb-cyan.vercel.app)"
fi
...
( cd "$APP_DIR" && vercel pull --yes --environment=production ) >"$PULL_LOG" 2>&1 || true

# Build prebuilt output
echo "Building prebuilt output… (log: $BUILD_LOG)"
( cd "$APP_DIR" && vercel build --prod ) >"$BUILD_LOG" 2>&1 || {
...
( cd "$APP_DIR" && vercel deploy --prebuilt --prod --yes ) >"$DEPLOY_LOG" 2>&1 || {
  echo "=== CTO VERCEL PREBUILT SUMMARY START ==="
  echo "Phase: deploy"
  echo "Result: FAIL"
...
# Extract the deployment URL (if printed)
...
echo "=== CTO VERCEL PREBUILT SUMMARY START ==="
echo "Phase: full (build+deploy+probe)"
echo "Result: DONE"
echo "Prod URL: $URL"
